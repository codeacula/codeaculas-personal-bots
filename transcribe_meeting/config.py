...
WHISPER_COMPUTE_TYPE = os.getenv("WHISPER_COMPUTE_TYPE", "int8") # Was "float16"
WHISPER_BEAM_SIZE = int(os.getenv("WHISPER_BEAM_SIZE", 4)) # Keep beam size at 4 from last test
WHISPER_BATCH_SIZE = int(os.getenv("WHISPER_BATCH_SIZE", 8)) # Set batch size based on benchmark

DIARIZATION_PIPELINE_NAME = os.getenv("DIARIZATION_PIPELINE_NAME", "pyannote/speaker-diarization-3.1")
HUGGINGFACE_AUTH_TOKEN = os.getenv("HUGGINGFACE_AUTH_TOKEN", None)

# --- Paths and File Management ---
# Use raw strings (r"...") for Windows paths if needed, but os.path.join handles separators
REPO_ROOT = os.getenv("REPO_ROOT", r"D:\Projects\work-notes") # Root of the Git repository
TRANSCRIPT_BASE_DIR_NAME = os.getenv("TRANSCRIPT_BASE_DIR_NAME", "Transcripts") # Subdirectory for transcripts within repo
PROCESSED_VIDEO_DIR = os.getenv("PROCESSED_VIDEO_DIR", r"D:\Processed") # Fixed directory for processed videos
DELETE_TEMP_AUDIO = os.getenv("DELETE_TEMP_AUDIO", "True").lower() in ("true", "1", "t") # Delete the intermediate WAV file?
MOVE_PROCESSED_VIDEO = os.getenv("MOVE_PROCESSED_VIDEO", "True").lower() in ("true", "1", "t") # Move the original video after processing?

# --- SRT Formatting Options ---
SRT_OPTIONS = {
    "max_line_length": int(os.getenv("SRT_MAX_LINE_LENGTH", 42)),
    "max_words_per_entry": int(os.getenv("SRT_MAX_WORDS_PER_ENTRY", 10)),
    "speaker_gap_threshold": float(os.getenv("SRT_SPEAKER_GAP_THRESHOLD", 1.0)), # Seconds gap to force new SRT entry
}

# --- Git Options ---
GIT_ENABLED = os.getenv("GIT_ENABLED", "True").lower() in ("true", "1", "t") # Enable/disable Git operations
GIT_COMMIT_MESSAGE_PREFIX = os.getenv("GIT_COMMIT_MESSAGE_PREFIX", "Add transcript for") # Prefix for commit messages

# --- Alignment Multiprocessing Tuning ---
# Target number of words for each parallel alignment task
ALIGNMENT_TARGET_WORDS_PER_CHUNK = int(os.getenv("ALIGNMENT_TARGET_WORDS_PER_CHUNK", 5000))
# Maximum number of worker processes to use for alignment (e.g., physical cores, or less)
ALIGNMENT_MAX_WORKERS = int(os.getenv("ALIGNMENT_MAX_WORKERS", 12)) # Set based on your preference/CPU (e.g., os.cpu_count() // 2)
